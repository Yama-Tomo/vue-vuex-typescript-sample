...
describe('filter todo', () => {
  test('todoが追加されること', async () => {
    ...
    if (lastTodoElement === undefined) throw new Error('failed get todo element');
    ...
    expect(todoElements.length).toEqual(todoElements.length - activeTodoElements.length);

    await topPage.filterCompletedTodo();
    const completedTodoElements = await topPage.getCompletedTodoElements();
    expect(todoElements.length).toEqual(todoElements.length - completedTodoElements.length);
  }, timeout);
});
