...
describe('create, update and delete of todo', () => {
  test('todoが追加されること', async () => {
    ...
    const todoContent = await (() => {
      ...
      if (lastElement === undefined) throw new Error('not found last element');
  ...
  test('todoを編集できること', async () => {
    ...
    if (beforeTodoElement === undefined) throw new Error('failed get todo element');
  ...
  test('todoを削除できること', async () => {
    ...
    if (beforeLastTodoElement === undefined) throw new Error('failed get todo element');
